...
def Chamaca(c_out, filters=[32, 64, 128, 256, 384, 1024, 640]):
...
def chamaca_splitter(m):
    return [list(p.parameters()) for p in m]

def load_chamaca(cats, file=None, **kwargs):
    if file is None:
        model = Chamaca(cats,**kwargs)
    else:
    ...
    print(f"Correctly loaded model Chamaca with {num_params(model)/10**6:.3f}M parameters")
    return model
